...
class AgentPipeline:
    ...
    async def run(self):
        ...
        try:
            ...
            logger.info("Cloning repository",
                extra={"job_id": self.job_id, "repo_url": self.repo_url})
            ...
            logger.info("Clone complete",
                extra={"job_id": self.job_id, "duration_s": clone_duration})

            results = {}

            # ── Stage 1: Parallel — Scanner + Dependency Analyzer ──────────
            logger.info("Stage 1 — parallel scan + dependency analysis",
            ...
            logger.info("Stage 2 — environment builder",
                extra={"job_id": self.job_id})
            ...
            logger.info("Stage 3 — parallel AI explain + test run",
                extra={"job_id": self.job_id})
            await job_store.record_concurrent_agents(self.job_id, 2)
    ...
    async def _run_agent(self, agent) -> dict:
        ...
        try:
            ...
            logger.error(f"Agent failed: {agent.name}",
                extra={"job_id": self.job_id, "error": str(e)})
